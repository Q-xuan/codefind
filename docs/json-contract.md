...
- `--timeout` maximum remains 10s. Raising the ceiling is not part of this contract. A directory xlsx request content-scans one ranked workbook; other files use coverage reason `deferred` and do not by themselves produce `budget_exceeded`. Follow up with `--path` on the next pending file. Named multi-file paths still scan each named file.
