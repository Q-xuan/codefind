...
- `--timeout` maximum remains 10s. Raising the ceiling is not part of this contract. A directory or single-file xlsx request content-scans one workbook. `workbook_coverage.files[]` includes additive `score` and `size`. `next_path` is the next ranked sibling after the scanned file. Other files use `reason=deferred` and do not by themselves produce `budget_exceeded`. Hop with `--path` on `next_path`; the deferred queue is kept. Named multi-file paths still scan each named file. Single-term ties are not broken by smaller-file-first.
