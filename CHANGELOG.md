...
- `--path` may name a single file or exclude one with a `!` prefix. Directory xlsx cold start ranks by filename, sheet names and shared-string literals, then content-scans only the top workbook (`deferred` for the rest). Timeout ceiling stays 10s; no synonym matching.
