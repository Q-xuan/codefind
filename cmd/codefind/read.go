...
func runRead(args []string, stdout, stderr io.Writer) int {
	...
	f.StringVar(&req.Range, "range", "", "明确范围，例如 A44:L55；范围已知时优先于 --field")
	...
	f.Var(&fields, "field", "按表头名猜列，可重复；邻近同名列可能选错，范围已知时用 --range")
	...
	if e := f.Parse(args); e != nil {
		if errors.Is(e, flag.ErrHelp) {
			return writeHelp(stdout, stderr, f, readHelpNotes)
		}
